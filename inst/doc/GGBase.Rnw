...
We have decided to use a dynamically exported SQLite connection
to provide access to SNP location information.

<<kjs>>=
dbListTables(hmceuAmbB36_23a_dbconn())
@

\section{Conclusion}

The concepts discussed above define the bulk of the tools
supporting the gwSnpScreen method of GGtools.  Consult the
